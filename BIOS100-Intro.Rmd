...
Try that yourself. Insert a code chunk, type in a new calculation, and run the code chunnk...
...
It is not obvious in the above figure, but two of the data points are sitting directly on top of each other. They have the exact same values of Girth and Height. If you hand count the points in the above figure you'll only count 30, and trees has 31 rows of data. After looking through the data frame I notived that it is occuring where Girth is equal to 18. I am going to use the filter() function to only show the rows where Girth is equal to 18. When using filter(), you have to use the double equal sign (==) because we want to test for those rows in which they are equal. We are not storing a value in a parameter; we are doing a test.
...
The above two rows have identical Girth and Height, even though they have slightly different Volume.


```{r}
trees |>
  ggplot(aes(x=Girth, y=Height)) +
  geom_point(shape = 1, size = 3, position = "jitter") + #use shape and size to change the shape and size of the points
  theme_classic()
```
...
To plot that data, we'll use ggplot again, but instead of geom_point() we'll use geom_bar(). Becuase of the way we've set up the data, we need to include the option stat="identity" so that it just plots what we give it and doesn't try to do any extra counting.
